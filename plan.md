...
- `MODEL_CONFIG.gfs.frameStepHours = 3` (`scripts/lib/modelview-runtime.js:27`) is not a UI hour authority: `use-frame-status.ts` uses it only as a manifest fallback, synthesizing planned hours when the manifest yields none. Since Phase 2 it is also read by `buildFullHoursForModel` (`scripts/lib/noaa-build/run-resolution.js`) to plan `--full-run` hour lists, so GFS full runs render on a 3-hourly cadence by design.
